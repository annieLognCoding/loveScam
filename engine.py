...
# Opening JSON file
f = open('archive/generated-english-irregular-verbs-master/irregular.verbs.build.json')
...
irregular_verbs = []
for word in data:
    list_past = data[word][0]
    for i in list_past:
        try: 
            ind = int(i)
            irregular_verbs.extend(list_past[i])
        except:
            continue
...
with open('irregular_verbs.pkl', 'wb') as f:
    pickle.dump(irregular_verbs, f)
